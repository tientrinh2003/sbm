...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`http://${piHost}:8000/api/monitoring/start`, {
      method: 'POST',
      headers: {
    ...
    console.error('Pi proxy start monitoring error:', error);
    
    return NextResponse.json(
      { 
        error: 'Failed to start Pi monitoring',
