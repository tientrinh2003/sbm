...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`http://${piHost}:8000/api/health`, {
      method: 'GET',
      headers: {
    ...
    console.error('Pi proxy health error:', error);
    return NextResponse.json(
      { error: 'Failed to connect to Pi server', details: error.message },
      { 
        status: 500,
        headers: {
          'Access-Control-Allow-Origin': '*',
